...
document.querySelector(".equalOperator").addEventListener('click', function () {
  if (!opr.includes(screen.value.slice(-1))) {
    screen.value = Function('return (' + screen.value + ')')().toFixed(2).replace(".00", "")
  } else {
    let sliceLC = screen.value.slice(-1)
    let sliceBLC = screen.value.slice(0, -1)
    screen.value = Function('return (' + sliceBLC + ')')().toFixed(2).replace(".00", "") + sliceLC
  }
});
